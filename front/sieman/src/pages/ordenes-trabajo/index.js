...
function OrdenesTrabajo() {
  ...
  const columns = [
    ...
    { Header: "Producto", accessor: "producto.nombre", align: "center" },
  ...
  const updateStatus = async (orden, estado) => {
    try {
      await axios.patch("http://localhost:8000/api/ordenes-trabajo/" + orden.id + "/", {
        estado: estado,
      });
      setIsUpdated(true);
      Swal.fire(
        "¡Actualizado!",
        "El estado de la orden de produccion ha sido actualizado correctamente.",
        "success"
      );
    } catch (error) {
      console.log(error);
      Swal.fire(
        "¡Error!",
        "El estado de la orden de produccion no ha podido ser actualizado.",
        "error"
      );
    }
  };

  const productionProcess = async (orden) => {
    try {
      ...
      let text = "La producción no ha podido ser registrada.";
      ...
      Swal.fire("¡Error registrando la produccion!", text, "error");
    }
  };
  ...
  const handleClick = (registro, act) => {
    const actions = {
      Registrar: {
        action: "registrar la producción de",
        method: () => productionProcess(registro),
      },
      Aprobar: {
        action: "aprobar",
  ...
  useEffect(() => {
    const fetchDataFromAPI = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:8000/api/ordenes-trabajo/");
        ...
        data.forEach((registro) => {
          ...
          let btns =
            registro.estado == "Pendiente" ? (
              <div>
                <Icon
                  title="APROBAR ORDEN DE PRODUCCION"
                  fontSize="small"
                  style={{
                ...
                <Icon
                  title="RECHAZAR ORDEN DE PRODUCCION"
                  fontSize="small"
                  style={{
                    color: "rgb(220 48 44)",
                  }}
              ...
              <div>
                <Icon
                  ...
                  className="fake-link"
                  onClick={() => handleClick(registro, "Registrar")}
                >
          ...
          registro.accion =
            registro.estado == "Pendiente" || registro.estado == "Aprobada" ? btns : "";
        });
            ...
            <Card>
              ...
              >
                <MDTypography variant="h6" color="white">
                  Ordenes de Producción Registradas
                </MDTypography>
                <Link
