...
function EditMateriasPrimas() {
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      axios
        .put("http://localhost:8000/api/materias-primas/" + id + "/", formData)
        .then((response) => {
          Swal.fire(
            "¡Guardado!",
            "La Materia Prima ha sido editada correctamente.",
            "success"
          ).then((result) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/materias-primas/" + id + "/")
      .then((response) => {
        setFormData({
          ...formData,
            ...
            <Card>
              ...
              >
                <MDTypography variant="h6" color="white">
                  Editando Materia Prima &apos;{formData.nombre}&apos;
                </MDTypography>
                <Link
                  to="/materias-primas"
                  style={{
